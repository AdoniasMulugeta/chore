<template>
    <div class="todo-item" :key="todo.key" v-bind="todo">
        ...
        <input v-if="todo.editing" class="todo-name" type="text" @keyup.enter="doneEditTodo" @blur="doneEditTodo" v-model="todo.name">
        <div ref="todo_Edit" v-else id="todo.id" class="todo-name"  @dblclick="editTodo">{{ todo.name }}</div>
